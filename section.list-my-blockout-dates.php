...
<div class="d-flex justify-content-between top-page-buttons">
  <button class="btn btn-sm btn-outline-primary px-2 ms-auto me-2" onclick="$(document).trigger('myblockout:reloadList');">
    <i class="fa-solid fa-arrow-rotate-right"></i>
    Reload
  </button>
  <button class=" btn btn-outline-primary btn-sm px-2" onclick="$(document).trigger('loadMainSection', { sectionId: 'blockouts', url: 'section.edit-my-blockout.php', forceReload: true });">
    New Block Out
  </button>
</div>

<div class="d-flex justify-content-between mt-2">
  <h2>My Block Out Dates</h2>
</div>

<table id="table-myblockouts" class="table table-striped">
  <thead>
    <tr class="table-dark">
...
<script>
  ...
  if (!documentEventExists('myblockout:reloadList')) {
    $(document).on('myblockout:reloadList', async (e) => {
  ...
  if (!documentEventExists('listActionItem:myblockout')) {
    $(document).on('listActionItem:myblockout', async (e, el) => {
      e.stopPropagation();
      ...
      const dropdownMenu = `
        <div id="${myRandomId}" data-id="${id}" class="dropdown-menu show shadow" style="position: absolute; left: ${offset.top}px; top: ${offset.left}px; z-index: 1000;">
          <button class="dropdown-item" onclick="$(document).trigger('myblockout:edit', ${id})"><i class="fa-duotone fa-regular fa-pen-to-square"></i> Edit</button>
          <hr class="dropdown-divider">
          <button class="dropdown-item text-danger" onclick="$(document).trigger('myblockout:delete', ${id})"><i class="fa-duotone fa-regular fa-trash"></i> Delete</button>
        </div>
      `;
  ...
  if (!documentEventExists('myblockout:edit')) {
    $(document).on('myblockout:edit', async (e, id) => {
      $(document).trigger('loadMainSection', {
        sectionId: 'myBlockouts',
        url: `section.edit-my-blockout.php?id=${id}`,
        forceReload: true
      });
    });
  }

  if (!documentEventExists('myblockout:delete')) {
    $(document).on('myblockout:delete', async (e, id) => {
      if (await ui.ask('Are you sure you want to delete this blockout date?')) {
        ...
        if (resp.result) {
          ...
          return $(document).trigger('myblockout:reloadList');
        }
  ...
  $(async ƒ => {
    ...
    const tableId = 'table-myblockouts';
    const dataTableOptions = {
      responsive: true
    };
    const reloadOnEventName = 'myblockoutChange';
